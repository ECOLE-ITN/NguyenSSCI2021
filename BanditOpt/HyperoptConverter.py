...
def ToHyperopt(search_space: list, sp: SearchSpace, con: ConditionalSpace, prefix='value'):
    Ls_hpOpt = []
...
'''def ToHyperopt(search_space: SearchSpace, AllConditional: dict(),BreakConditional:dict()):
    ...
    for root in lsRootNode:
        root'''
def ToHyperopt2(search_space: SearchSpace, con: ConditionalSpace, prefix='value'):
    lsParentName, childList, lsFinalSP, ActiveLst, noCheckForb = [], [], [], [], []
...
if __name__ == '__main__':
    ...
    hpos=ToHyperopt2(search_space,con)
    searchSpace = search_space.Combine(con, fobr, True)
    i=0
    while (i<10):
        i+=1
